...
#     sort -u | grep -v -F -f ${dictdir}/silence_phones.txt >${dictdir}/nonsilence_phones.txt

# Now use the given phone_table instead:
cut -f1 -d" " <"$phone_table" | sed -e 's/_.$//g' -e '/^\#/d' -e '/^<eps>/d' |
    sort -u | grep -v -F -f ${dictdir}/silence_phones.txt >${dictdir}/nonsilence_phones.txt
...
# Add just <unk> to placeholder lexicon.
# The sentencepiece space is handled by subword-kaldi/local/make_lfst_spm.py
echo -e "$unk\tSPN" >>${dictdir}/lexicon_placeholders.txt

extra=5 #Need 5 extra disambig symbols for sentencepiece
utils/prepare_lang.sh --phone_symbol_table "$phone_table" $prepare_lang_opts \
    --num-extra-phone-disambig-syms $extra ${dictdir} "$unk" ${langdir}/local ${langdir}
...
subword-kaldi/local/make_lfst_spm.py $(tail -n$extra ${langdir}/phones/disambig.txt) \
    ...
    fstcompile --isymbols=${langdir}/phones.txt --osymbols=${langdir}/words.txt \
