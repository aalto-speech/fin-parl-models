...
if [ $stage -le 2 ]; then
	...
	# Prep lexicon
	local/prepare_lexicon.sh ${traindir} data/andre_comparison/local/dict_train_cleaned data/andre_comparison/lang_train_cleaned
fi

utils/lang/check_phones_compatible.sh data/andre_comparison/lang_train_cleaned/phones.txt data/lang_train/phones.txt
...
if [ $stage -le 6 ]; then
  ...
  steps/train_mono.sh --boost-silence 1.25 --nj 20 --cmd "$basic_cmd" \
                      ${traindir}_2kshort data/andre_comparison/lang_train_cleaned exp/andre_comparison/i_cleaned/mono
fi

if [ $stage -le 7 ]; then
	echo "Stage 7: Evaluate monophone system."
	utils/mkgraph.sh data/lang_$lm exp/andre_comparison/i_cleaned/mono exp/andre_comparison/i_cleaned/mono/graph_$lm

	# Decode with triphone model
	steps/decode.sh --cmd "$basic_cmd" --nj 8 \
									exp/andre_comparison/i_cleaned/mono/graph_$lm data/parl-dev-all \
									exp/andre_comparison/i_cleaned/mono/decode_parl-dev-all_$lm
fi

if [ $stage -le 8 ]; then
	echo "Stage 8: Train first triphone system with 100k utterances."
  steps/align_si.sh --boost-silence 1.25 --nj 10 --cmd "$basic_cmd" \
                    ${traindir}_100k data/andre_comparison/lang_train_cleaned exp/andre_comparison/i_cleaned/mono exp/andre_comparison/i_cleaned/mono_ali_100k
  ...
  steps/train_deltas.sh --boost-silence 1.25 --cmd "$basic_cmd" \
                        2000 10000 ${traindir}_100k data/andre_comparison/lang_train_cleaned exp/andre_comparison/i_cleaned/mono_ali_100k exp/andre_comparison/i_cleaned/tri1a
fi

if [ $stage -le 9 ]; then
	echo "Stage 9: Evaluate 100k triphone system."
	utils/mkgraph.sh data/lang_$lm exp/andre_comparison/i_cleaned/tri1a exp/andre_comparison/i_cleaned/tri1a/graph_$lm

	# Decode with triphone model
	steps/decode.sh --cmd "$basic_cmd" --nj 8 \
									exp/andre_comparison/i_cleaned/tri1a/graph_$lm data/parl-dev-all \
									exp/andre_comparison/i_cleaned/tri1a/decode_parl-dev-all_$lm
fi

if [ $stage -le 10 ]; then
	echo "Stage 10: Train second triphone system with 250k utterances."
	steps/align_si.sh --nj 40 --cmd "$basic_cmd" ${traindir}_250k data/andre_comparison/lang_train_cleaned exp/andre_comparison/i_cleaned/tri1a exp/andre_comparison/i_cleaned/tri1a_ali_250k

	# train an LDA+MLLT system
	steps/train_lda_mllt.sh --cmd "$basic_cmd" --splice-opts "--left-context=3 --right-context=3" 4000 45000 \
													${traindir}_250k data/andre_comparison/lang_train_cleaned exp/andre_comparison/i_cleaned/tri1a_ali_250k exp/andre_comparison/i_cleaned/tri2a
fi

if [ $stage -le 11 ]; then
	echo "Stage 11: Evaluate second triphone system."
	utils/mkgraph.sh data/lang_$lm exp/andre_comparison/i_cleaned/tri2a exp/andre_comparison/i_cleaned/tri2a/graph_$lm

	# Decode with triphone model
	steps/decode.sh --cmd "$basic_cmd" --nj 8 \
									exp/andre_comparison/i_cleaned/tri2a/graph_$lm data/parl-dev-all \
									exp/andre_comparison/i_cleaned/tri2a/decode_parl-dev-all_$lm
fi
...
if [ $stage -le 12 ]; then
	...
	steps/align_si.sh --nj 40 --cmd "$basic_cmd" --use-graphs true \
										${traindir}_250k data/andre_comparison/lang_train_cleaned exp/andre_comparison/i_cleaned/tri2a exp/andre_comparison/i_cleaned/tri2a_ali_250k

	# train an LDA+MLLT+SAT system
	steps/train_sat.sh --cmd "$basic_cmd" \
													4000 45000 \
													${traindir}_250k data/andre_comparison/lang_train_cleaned exp/andre_comparison/i_cleaned/tri2a_ali_250k exp/andre_comparison/i_cleaned/tri3a
fi

if [ $stage -le 13 ]; then
	echo "Stage 13: Evaluate third triphone system."
	utils/mkgraph.sh data/lang_$lm exp/andre_comparison/i_cleaned/tri3a exp/andre_comparison/i_cleaned/tri3a/graph_$lm

	# Decode with triphone model
	steps/decode_fmllr.sh --cmd "$basic_cmd" --nj 16 \
									exp/andre_comparison/i_cleaned/tri3a/graph_$lm data/parl-dev-all \
									exp/andre_comparison/i_cleaned/tri3a/decode_parl-dev-all_$lm
fi
...
if [ $stage -le 14 ]; then
	...
	if [ ! -d exp/andre_comparison/i_cleaned/tri3a_ali_full ]; then
		steps/align_fmllr.sh --nj 40 --cmd "$basic_cmd" \
										${traindir} data/andre_comparison/lang_train_cleaned exp/andre_comparison/i_cleaned/tri3a exp/andre_comparison/i_cleaned/tri3a_ali_full
	fi
	...
	steps/train_sat.sh --cmd "$basic_cmd" --num-iters $num_iters \
													...
													${traindir} data/andre_comparison/lang_train_cleaned exp/andre_comparison/i_cleaned/tri3a_ali_full exp/andre_comparison/i_cleaned/tri4$tri_version
fi

if [ $stage -le 15 ]; then
	echo "Stage 15: Evaluate fourth triphone system."
	utils/mkgraph.sh data/lang_$lm exp/andre_comparison/i_cleaned/tri4$tri_version exp/andre_comparison/i_cleaned/tri4$tri_version/graph_$lm

	# Decode with triphone model
	steps/decode_fmllr.sh --cmd "$basic_cmd" --nj 16 \
									exp/andre_comparison/i_cleaned/tri4$tri_version/graph_$lm data/parl-dev-all \
									exp/andre_comparison/i_cleaned/tri4$tri_version/decode_parl-dev-all_$lm
fi
