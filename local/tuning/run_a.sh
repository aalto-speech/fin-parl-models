...
if [ $stage -le 6 ]; then
    ...
    steps/train_mono.sh --boost-silence 1.25 --nj 8 --cmd "$basic_cmd" \
        ${traindir}_2kshort data/lang_${trainset} exp/${trainset}/a/mono
fi

if [ $stage -le 7 ]; then
    echo "Stage 7: Evaluate monophone system."
    utils/mkgraph.sh $lmdir exp/${trainset}/a/mono \
        exp/${trainset}/a/mono/graph_${lm_name}

    steps/decode.sh --cmd "$basic_cmd" --nj 8 \
        exp/${trainset}/a/mono/graph_${lm_name} data/parl2016-dev \
        exp/${trainset}/a/mono/decode_parl2016-dev_${lm_name}
fi
...
if [ $stage -le 8 ]; then
    ...
    steps/align_si.sh --boost-silence 1.25 --nj 8 --cmd "$basic_cmd" \
        ...
        exp/${trainset}/a/mono exp/${trainset}/a/mono_ali_5k

    steps/train_deltas.sh --boost-silence 1.25 --cmd "$basic_cmd" \
        2000 10000 ${traindir}_5k data/lang_${trainset} \
        exp/${trainset}/a/mono_ali_5k exp/${trainset}/a/tri1a
fi

if [ $stage -le 9 ]; then
    echo "Stage 9: Evaluate 5k triphone system."
    utils/mkgraph.sh $lmdir exp/${trainset}/a/tri1a \
        exp/${trainset}/a/tri1a/graph_${lm_name}

    steps/decode.sh --cmd "$basic_cmd" --nj 8 \
        exp/${trainset}/a/tri1a/graph_${lm_name} data/parl2016-dev \
        exp/${trainset}/a/tri1a/decode_parl2016-dev_${lm_name}
fi

if [ $stage -le 10 ]; then
    echo "Stage 10: Train second triphone system with 10k utterances."
    steps/align_si.sh --nj 8 --cmd "$basic_cmd" ${traindir}_10k \
        data/lang_${trainset} exp/${trainset}/a/tri1a \
        exp/${trainset}/a/tri1a_ali_10k

    steps/train_lda_mllt.sh --cmd "$basic_cmd" \
        --splice-opts "--left-context=3 --right-context=3" \
        2500 15000 ${traindir}_10k data/lang_${trainset} \
        exp/${trainset}/a/tri1a_ali_10k exp/${trainset}/a/tri2a
fi

if [ $stage -le 11 ]; then
    echo "Stage 11: Evaluate second triphone system."
    utils/mkgraph.sh $lmdir exp/${trainset}/a/tri2a \
        exp/${trainset}/a/tri2a/graph_${lm_name}

    steps/decode.sh --cmd "$basic_cmd" --nj 8 \
        exp/${trainset}/a/tri2a/graph_${lm_name} data/parl2016-dev \
        exp/${trainset}/a/tri2a/decode_parl2016-dev_${lm_name}
fi
...
if [ $stage -le 12 ]; then
    ...
    steps/align_si.sh --nj 8 --cmd "$basic_cmd" --use-graphs true \
        ...
        exp/${trainset}/a/tri2a exp/${trainset}/a/tri2a_ali_10k

    steps/train_sat.sh --cmd "$basic_cmd" \
        2500 15000 ${traindir}_10k data/lang_${trainset} \
        exp/${trainset}/a/tri2a_ali_10k exp/${trainset}/a/tri3a
fi

if [ $stage -le 13 ]; then
    echo "Stage 13: Evaluate third triphone system."
    utils/mkgraph.sh $lmdir exp/${trainset}/a/tri3a \
        exp/${trainset}/a/tri3a/graph_${lm_name}

    steps/decode_fmllr.sh --cmd "$basic_cmd" --nj 8 \
        exp/${trainset}/a/tri3a/graph_${lm_name} data/parl2016-dev \
        exp/${trainset}/a/tri3a/decode_parl2016-dev_${lm_name}
fi
...
if [ $stage -le 14 ]; then
    ...
    if [ ! -d exp/${trainset}/a/tri3a_ali_full ]; then
        steps/align_fmllr.sh --nj 8 --cmd "$basic_cmd" \
            $traindir data/lang_${trainset} \
            exp/${trainset}/a/tri3a exp/${trainset}/a/tri3a_ali_full
    fi
    ...
    steps/train_sat.sh --cmd "$basic_cmd" --num-iters $num_iters \
        ...
        exp/${trainset}/a/tri3a_ali_full exp/${trainset}/a/tri4$tri_version
fi

if [ $stage -le 15 ]; then
    echo "Stage 15: Evaluate fourth triphone system."
    utils/mkgraph.sh $lmdir exp/${trainset}/a/tri4$tri_version \
        exp/${trainset}/a/tri4$tri_version/graph_${lm_name}

    steps/decode_fmllr.sh --cmd "$basic_cmd" --nj 8 \
        exp/${trainset}/a/tri4$tri_version/graph_${lm_name} data/parl2016-dev \
        exp/${trainset}/a/tri4$tri_version/decode_parl2016-dev_${lm_name}
fi
