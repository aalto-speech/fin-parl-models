...
if [ $stage -le 6 ]; then
  ...
  steps/train_mono.sh --boost-silence 1.25 --nj 20 --cmd "$basic_cmd" \
                      data/train_2kshort data/lang_train exp/a/mono
fi

if [ $stage -le 7 ]; then
	echo "Stage 7: Evaluate monophone system."
	utils/mkgraph.sh data/lang_test_small exp/a/mono exp/a/mono/graph_test_small

	# Decode with triphone model
	steps/decode.sh --cmd "$basic_cmd" --nj 8 \
									exp/a/mono/graph_test_small data/parl-dev-all \
									exp/a/mono/decode_parl-dev-all_test_small
fi

if [ $stage -le 8 ]; then
	echo "Stage 8: Train first triphone system with 5k utterances."
  steps/align_si.sh --boost-silence 1.25 --nj 10 --cmd "$basic_cmd" \
                    data/train_5k data/lang_train exp/a/mono exp/a/mono_ali_5k

  # train a first delta + delta-delta triphone system on a subset of 5000 utterances
  steps/train_deltas.sh --boost-silence 1.25 --cmd "$basic_cmd" \
                        2000 10000 data/train_5k data/lang_train exp/a/mono_ali_5k exp/a/tri1a
fi

if [ $stage -le 9 ]; then
	echo "Stage 9: Evaluate 5k triphone system."
	utils/mkgraph.sh data/lang_test_small exp/a/tri1a exp/a/tri1a/graph_test_small

	# Decode with triphone model
	steps/decode.sh --cmd "$basic_cmd" --nj 8 \
									exp/a/tri1a/graph_test_small data/parl-dev-all \
									exp/a/tri1a/decode_parl-dev-all_test_small
fi

if [ $stage -le 10 ]; then
	echo "Stage 10: Train second triphone system with 10k utterances."
	steps/align_si.sh --nj 10 --cmd "$basic_cmd" data/train_10k data/lang_train exp/a/tri1a exp/a/tri1a_ali_10k

	# train an LDA+MLLT system
	steps/train_lda_mllt.sh --cmd "$basic_cmd" --splice-opts "--left-context=3 --right-context=3" 2500 15000 \
													data/train_10k data/lang_train exp/a/tri1a_ali_10k exp/a/tri2a
fi

if [ $stage -le 11 ]; then
	echo "Stage 11: Evaluate second triphone system."
	utils/mkgraph.sh data/lang_test_small exp/a/tri2a exp/a/tri2a/graph_test_small

	# Decode with triphone model
	steps/decode.sh --cmd "$basic_cmd" --nj 8 \
									exp/a/tri2a/graph_test_small data/parl-dev-all \
									exp/a/tri2a/decode_parl-dev-all_test_small
fi
...
if [ $stage -le 12 ]; then
	...
	steps/align_si.sh --nj 10 --cmd "$basic_cmd" --use-graphs true \
										data/train_10k data/lang_train exp/a/tri2a exp/a/tri2a_ali_10k

	# train an LDA+MLLT+SAT system
	steps/train_sat.sh --cmd "$basic_cmd" \
													2500 15000 \
													data/train_10k data/lang_train exp/a/tri2a_ali_10k exp/a/tri3a
fi

if [ $stage -le 13 ]; then
	echo "Stage 13: Evaluate third triphone system."
	utils/mkgraph.sh data/lang_test_small exp/a/tri3a exp/a/tri3a/graph_test_small

	# Decode with triphone model
	steps/decode_fmllr.sh --cmd "$basic_cmd" --nj 16 \
									exp/a/tri3a/graph_test_small data/parl-dev-all \
									exp/a/tri3a/decode_parl-dev-all_test_small
fi
...
if [ $stage -le 14 ]; then
	...
	if [ ! -d exp/a/tri3a_ali_full ]; then
		steps/align_fmllr.sh --nj 120 --cmd "$basic_cmd" \
										data/train data/lang_train exp/a/tri3a exp/a/tri3a_ali_full
	fi
	...
	steps/train_sat.sh --cmd "$basic_cmd" --num-iters $num_iters \
													...
													data/train data/lang_train exp/a/tri3a_ali_full exp/a/tri4$tri_version
fi

if [ $stage -le 15 ]; then
	echo "Stage 15: Evaluate fourth triphone system."
	utils/mkgraph.sh data/lang_test_small exp/a/tri4$tri_version exp/a/tri4$tri_version/graph_test_small

	# Decode with triphone model
	steps/decode_fmllr.sh --cmd "$basic_cmd" --nj 16 \
									exp/a/tri4$tri_version/graph_test_small data/parl-dev-all \
									exp/a/tri4$tri_version/decode_parl-dev-all_test_small
fi
