...
if [ $stage -le 6 ]; then
    ...
    steps/train_mono.sh --boost-silence 1.25 --nj 8 --cmd "$basic_cmd" \
        ${traindir}_2kshort data/lang_${trainset} exp/${trainset}/d/mono
fi

if [ $stage -le 7 ]; then
    echo "Stage 7: Evaluate monophone system."
    utils/mkgraph.sh $lmdir exp/${trainset}/d/mono \
        exp/${trainset}/d/mono/graph_${lm_name}

    steps/decode.sh --cmd "$basic_cmd" --nj 8 \
        exp/${trainset}/d/mono/graph_${lm_name} data/parl2016-dev \
        exp/${trainset}/d/mono/decode_parl2016-dev_${lm_name}
fi

if [ $stage -le 8 ]; then
    echo "Stage 8: Train first triphone system with 250k utterances."
    steps/align_si.sh --nj 8 --cmd "$basic_cmd" ${traindir}_250k \
        data/lang_${trainset} exp/${trainset}/d/mono \
        exp/${trainset}/d/mono_ali_250k

    steps/train_lda_mllt.sh --cmd "$basic_cmd" \
        --splice-opts "--left-context=3 --right-context=3" \
        4000 45000 ${traindir}_250k data/lang_${trainset} \
        exp/${trainset}/d/mono_ali_250k exp/${trainset}/d/tri1a
fi

if [ $stage -le 9 ]; then
    echo "Stage 9: Evaluate first triphone system."
    utils/mkgraph.sh $lmdir exp/${trainset}/d/tri1a \
        exp/${trainset}/d/tri1a/graph_${lm_name}

    steps/decode.sh --cmd "$basic_cmd" --nj 8 \
        exp/${trainset}/d/tri1a/graph_${lm_name} data/parl2016-dev \
        exp/${trainset}/d/tri1a/decode_parl2016-dev_${lm_name}
fi
...
if [ $stage -le 10 ]; then
    ...
    steps/train_sat.sh --cmd "$basic_cmd" --num-iters $num_iters \
        ...
        exp/${trainset}/d/tri1a_ali exp/${trainset}/d/tri2$tri_version
fi

if [ $stage -le 11 ]; then
    echo "Stage 11: Evaluate second triphone system."
    utils/mkgraph.sh $lmdir exp/${trainset}/d/tri2$tri_version \
        exp/${trainset}/d/tri2$tri_version/graph_${lm_name}

    steps/decode_fmllr.sh --cmd "$basic_cmd" --nj 8 \
        exp/${trainset}/d/tri2$tri_version/graph_${lm_name} data/parl2016-dev \
        exp/${trainset}/d/tri2$tri_version/decode_parl2016-dev_${lm_name}
fi
