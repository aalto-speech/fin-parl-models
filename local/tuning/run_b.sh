...
if [ $stage -le 6 ]; then
  ...
  steps/train_mono.sh --boost-silence 1.25 --nj 120 --cmd "$basic_cmd" \
                      data/train data/lang_train exp/b/mono
fi

if [ $stage -le 7 ]; then
	echo "Stage 7: Evaluate monophone system."
	utils/mkgraph.sh data/lang_test_small exp/b/mono exp/b/mono/graph_test_small

	# Decode with monophone model
	steps/decode.sh --cmd "$basic_cmd" --nj 16 \
									exp/b/mono/graph_test_small data/parl-dev-all \
									exp/b/mono/decode_parl-dev-all_test_small
fi

if [ $stage -le 10 ]; then
	echo "Stage 10: Train first triphone system."
	steps/align_si.sh --nj 120 --cmd "$basic_cmd" data/train data/lang_train exp/b/mono exp/b/mono_ali

	# train an LDA+MLLT system
	steps/train_lda_mllt.sh --cmd "$basic_cmd" --splice-opts "--left-context=3 --right-context=3" 2500 15000 \
													data/train data/lang_train exp/b/mono_ali exp/b/tri1a
fi

if [ $stage -le 11 ]; then
	echo "Stage 11: Evaluate first triphone system."
	utils/mkgraph.sh data/lang_test_small exp/b/tri1a exp/b/tri1a/graph_test_small

	# Decode with triphone model
	steps/decode.sh --cmd "$basic_cmd" --nj 16 \
									exp/b/tri1a/graph_test_small data/parl-dev-all \
									exp/b/tri1a/decode_parl-dev-all_test_small
fi

if [ $stage -le 12 ]; then
	echo "Stage 12: Train second triphone system with LDA+MLLT+SAT."
	steps/align_si.sh --nj 120 --cmd "$basic_cmd" --use-graphs true \
										data/train data/lang_train exp/b/tri1a exp/b/tri1a_ali

	# train an LDA+MLLT+SAT system
	steps/train_sat.sh --cmd "$basic_cmd" \
													7000 150000 \
													data/train data/lang_train exp/b/tri1a_ali exp/b/tri2a
fi

if [ $stage -le 13 ]; then
	echo "Stage 13: Evaluate second triphone system."
	utils/mkgraph.sh data/lang_test_small exp/b/tri2a exp/b/tri2a/graph_test_small

	# Decode with triphone model
	steps/decode_fmllr.sh --cmd "$basic_cmd" --nj 16 \
									exp/b/tri2a/graph_test_small data/parl-dev-all \
									exp/b/tri2a/decode_parl-dev-all_test_small
fi
