...
# This script trains the best HMM-GMM model on
# runs the first iteration of the experiments
...
if [ $stage -le 6 ]; then
    ...
    steps/train_mono.sh --boost-silence 1.25 --nj 8 --cmd "$basic_cmd" \
        ${traindir}_2kshort data/lang_${trainset} exp/${trainset}/i/mono
fi

if [ $stage -le 7 ]; then
    echo "Stage 7: Evaluate monophone system."
    utils/mkgraph.sh $lmdir exp/${trainset}/i/mono \
        exp/${trainset}/i/mono/graph_${lm_name}

    steps/decode.sh --cmd "$basic_cmd" --nj 8 \
        exp/${trainset}/i/mono/graph_${lm_name} data/parl2016-dev \
        exp/${trainset}/i/mono/decode_parl2016-dev_${lm_name}
fi
...
if [ $stage -le 8 ]; then
    ...
    steps/align_si.sh --boost-silence 1.25 --nj 8 --cmd "$basic_cmd" \
        ...
        exp/${trainset}/i/mono exp/${trainset}/i/mono_ali_100k

    steps/train_deltas.sh --boost-silence 1.25 --cmd "$basic_cmd" \
        2000 10000 ${traindir}_100k data/lang_${trainset} \
        exp/${trainset}/i/mono_ali_100k exp/${trainset}/i/tri1a
fi

if [ $stage -le 9 ]; then
    echo "Stage 9: Evaluate 100k triphone system."
    utils/mkgraph.sh $lmdir exp/${trainset}/i/tri1a \
        exp/${trainset}/i/tri1a/graph_${lm_name}

    steps/decode.sh --cmd "$basic_cmd" --nj 8 \
        exp/${trainset}/i/tri1a/graph_${lm_name} data/parl2016-dev \
        exp/${trainset}/i/tri1a/decode_parl2016-dev_${lm_name}
fi

if [ $stage -le 10 ]; then
    echo "Stage 10: Train second triphone system with 250k utterances."
    steps/align_si.sh --nj 8 --cmd "$basic_cmd" ${traindir}_250k \
        data/lang_${trainset} exp/${trainset}/i/tri1a \
        exp/${trainset}/i/tri1a_ali_250k

    steps/train_lda_mllt.sh --cmd "$basic_cmd" \
        --splice-opts "--left-context=3 --right-context=3" \
        4000 45000 ${traindir}_250k data/lang_${trainset} \
        exp/${trainset}/i/tri1a_ali_250k exp/${trainset}/i/tri2a
fi

if [ $stage -le 11 ]; then
    echo "Stage 11: Evaluate second triphone system."
    utils/mkgraph.sh $lmdir exp/${trainset}/i/tri2a \
        exp/${trainset}/i/tri2a/graph_${lm_name}

    steps/decode.sh --cmd "$basic_cmd" --nj 8 \
        exp/${trainset}/i/tri2a/graph_${lm_name} data/parl2016-dev \
        exp/${trainset}/i/tri2a/decode_parl2016-dev_${lm_name}
fi
...
if [ $stage -le 12 ]; then
    ...
    steps/align_si.sh --nj 8 --cmd "$basic_cmd" --use-graphs true \
        ...
        exp/${trainset}/i/tri2a exp/${trainset}/i/tri2a_ali_250k

    steps/train_sat.sh --cmd "$basic_cmd" \
        4000 45000 ${traindir}_250k data/lang_${trainset} \
        exp/${trainset}/i/tri2a_ali_250k exp/${trainset}/i/tri3a
fi

if [ $stage -le 13 ]; then
    echo "Stage 13: Evaluate third triphone system."
    utils/mkgraph.sh $lmdir exp/${trainset}/i/tri3a \
        exp/${trainset}/i/tri3a/graph_${lm_name}

    steps/decode_fmllr.sh --cmd "$basic_cmd" --nj 8 \
        exp/${trainset}/i/tri3a/graph_${lm_name} data/parl2016-dev \
        exp/${trainset}/i/tri3a/decode_parl2016-dev_${lm_name}
fi
...
if [ $stage -le 14 ]; then
    ...
    if [ ! -d exp/${trainset}/i/tri3a_ali_full ]; then
        steps/align_fmllr.sh --nj 8 --cmd "$basic_cmd" \
            $traindir data/lang_${trainset} \
            exp/${trainset}/i/tri3a exp/${trainset}/i/tri3a_ali_full
    fi
    ...
    steps/train_sat.sh --cmd "$basic_cmd" --num-iters $num_iters \
        ...
        exp/${trainset}/i/tri3a_ali_full exp/${trainset}/i/tri4$tri_version
fi

if [ $stage -le 15 ]; then
    echo "Stage 15: Evaluate fourth triphone system."
    utils/mkgraph.sh $lmdir exp/${trainset}/i/tri4$tri_version \
        exp/${trainset}/i/tri4$tri_version/graph_${lm_name}

    steps/decode_fmllr.sh --cmd "$basic_cmd" --nj 8 \
        exp/${trainset}/i/tri4$tri_version/graph_${lm_name} data/parl2016-dev \
        exp/${trainset}/i/tri4$tri_version/decode_parl2016-dev_${lm_name}
fi
