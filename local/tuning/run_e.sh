...
# This script runs the fifth iteration of the experiments where we train
# one monophone model with 2k_short samples,
# two triphones with 250k, and
...
if [ $stage -le 6 ]; then
    ...
    steps/train_mono.sh --boost-silence 1.25 --nj 8 --cmd "$basic_cmd" \
        ${traindir}_2kshort data/lang_${trainset} exp/${trainset}/e/mono
fi

if [ $stage -le 7 ]; then
    echo "Stage 7: Evaluate monophone system."
    utils/mkgraph.sh $lmdir exp/${trainset}/e/mono \
        exp/${trainset}/e/mono/graph_${lm_name}

    steps/decode.sh --cmd "$basic_cmd" --nj 8 \
        exp/${trainset}/e/mono/graph_${lm_name} data/parl2016-dev \
        exp/${trainset}/e/mono/decode_parl2016-dev_${lm_name}
fi

if [ $stage -le 8 ]; then
    echo "Stage 8: Train first triphone system with 250k utterances."
    steps/align_si.sh --nj 8 --cmd "$basic_cmd" ${traindir}_250k \
        data/lang_${trainset} exp/${trainset}/e/mono \
        exp/${trainset}/e/mono_ali_250k

    steps/train_lda_mllt.sh --cmd "$basic_cmd" \
        --splice-opts "--left-context=3 --right-context=3" \
        4000 45000 ${traindir}_250k data/lang_${trainset} \
        exp/${trainset}/e/mono_ali_250k exp/${trainset}/e/tri1a
fi

if [ $stage -le 9 ]; then
    echo "Stage 9: Evaluate first triphone system."
    utils/mkgraph.sh $lmdir exp/${trainset}/e/tri1a \
        exp/${trainset}/e/tri1a/graph_${lm_name}

    steps/decode.sh --cmd "$basic_cmd" --nj 8 \
        exp/${trainset}/e/tri1a/graph_${lm_name} data/parl2016-dev \
        exp/${trainset}/e/tri1a/decode_parl2016-dev_${lm_name}
fi

if [ $stage -le 10 ]; then
    echo "Stage 10: Train second triphone system with 250k utts and LDA+MLLT+SAT."
    steps/align_si.sh --nj 8 --cmd "$basic_cmd" --use-graphs true \
        ${traindir}_250k data/lang_${trainset} \
        exp/${trainset}/e/tri1a exp/${trainset}/e/tri1a_ali_250k

    steps/train_sat.sh --cmd "$basic_cmd" \
        4000 45000 ${traindir}_250k data/lang_${trainset} \
        exp/${trainset}/e/tri1a_ali_250k exp/${trainset}/e/tri2a
fi

if [ $stage -le 11 ]; then
    echo "Stage 11: Evaluate second triphone system."
    utils/mkgraph.sh $lmdir exp/${trainset}/e/tri2a \
        exp/${trainset}/e/tri2a/graph_${lm_name}

    steps/decode_fmllr.sh --cmd "$basic_cmd" --nj 8 \
        exp/${trainset}/e/tri2a/graph_${lm_name} data/parl2016-dev \
        exp/${trainset}/e/tri2a/decode_parl2016-dev_${lm_name}
fi

if [ $stage -le 12 ]; then
    echo "Stage 12: Train third triphone system with LDA+MLLT+SAT."
    steps/align_fmllr.sh --nj 8 --cmd "$basic_cmd" \
        $traindir data/lang_${trainset} \
        exp/${trainset}/e/tri2a exp/${trainset}/e/tri2a_ali

    steps/train_sat.sh --cmd "$basic_cmd" \
        7000 150000 $traindir data/lang_${trainset} \
        exp/${trainset}/e/tri2a_ali exp/${trainset}/e/tri3a
fi

if [ $stage -le 13 ]; then
    echo "Stage 13: Evaluate third triphone system."
    utils/mkgraph.sh $lmdir exp/${trainset}/e/tri3a \
        exp/${trainset}/e/tri3a/graph_${lm_name}

    steps/decode_fmllr.sh --cmd "$basic_cmd" --nj 8 \
        exp/${trainset}/e/tri3a/graph_${lm_name} data/parl2016-dev \
        exp/${trainset}/e/tri3a/decode_parl2016-dev_${lm_name}
fi
