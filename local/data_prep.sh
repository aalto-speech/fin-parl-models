...
for set in 2008-2016-train 2015-2020-train 2016-dev 2016-test 2020-test; do
    ...
    find $raw_data_dir -iname "*.trn" -type f -print0 |
        sort -zV | xargs -0 -I % sh -c 'echo -n %" "; cat %' >$dir/text || exit 1

    echo "Creating $dir/wav.scp."
    find $raw_data_dir -iname "*.wav" -type f -print0 |
        sort -zV | xargs -0 -I % sh -c 'echo %" "$PWD/%' >$dir/wav.scp || exit 1
    ...
    sed 's/.*\/\(.*\)\.trn /\1 /' $dir/text |
        awk '{ gsub("_","-",$1); print $0 }' |
        sponge $dir/text

    sed 's/.*\/\(.*\)\.wav /\1 /' $dir/wav.scp |
        awk '{ gsub("_","-",$1); print $0 }' |
        sponge $dir/wav.scp
